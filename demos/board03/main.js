...
function bootstrap() {
  ...
  part.addEventListener("mousedown", onMouseDown, {
    passive: false,
    useCapture: false,
  });
  ...
  for (let x = STEP; x < width - STEP + 2; x += STEP) {
    //this magic 2 is slightly confusing
...
function handlePartDown(evt) {
  ...
  document.addEventListener("mousemove", handlePartMove, {
    passive: false,
    useCapture: false,
  });
  document.addEventListener("mouseup", handlePartUp, {
    passive: false,
    useCapture: false,
  });

  newpart = createElement("div", ["part"], null, [
...
function handlePartUp(evt) {
  newpart.addEventListener("mousedown", onMouseDown, {
    passive: false,
    useCapture: false,
  }); //funny; recursive
